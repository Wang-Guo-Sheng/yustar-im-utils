...
因为Rime不支持发送退格键，必须用C外部库发送。在`rime-lua-sendKeyCode-main` （修改自 https://github.com/qiuyue0/rime-lua-sendKeyCode ）下用两个makefile分别编译一次，把生成的两个dll放在合适位置，dll目录粘贴到`cycle_select.lua`中
