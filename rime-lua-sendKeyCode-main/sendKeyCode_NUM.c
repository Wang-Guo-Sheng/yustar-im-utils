...
static void press_key(const int keyCode, int count) {
#if defined(_WIN32)
    ...
    ip.ki.time = 0;
    ip.ki.dwExtraInfo = 0;
    ip.ki.wVk = keyCode;//VkKeyScan(*keyName);  
    ...
    KeySym keySym = getKeySym(keyCode);
...
static int l_press_key(lua_State *L) {
    int vk;
    if (lua_isnumber(L, 1)) {
        vk = luaL_checkinteger(L, 1);
    } else {
        const char *keyName = luaL_checkstring(L, 1);
        vk = getKeyCode(keyName);
    }
