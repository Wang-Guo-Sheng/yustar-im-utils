...
SHORT getKeyCode(const char* keyName) {
    ...
    // Windows 版本
    if (strcmp(keyName, "\b") == 0)          return VK_BACK;
...
#else
KeySym getKeySym(const char* keyName) {
    if (keyName == NULL) {
        return -1; // Invalid key name
    }
    // Linux 
    if (strcmp(keyName, "\b") == 0)             return XK_BackSpace;
    if (strcmp(keyName, "SHIFT") == 0)          return XK_Shift_L;
    if (strcmp(keyName, "CONTROL") == 0)        return XK_Control_L;
    if (strcmp(keyName, "CTRL") == 0)           return XK_Control_L;
    if (strcmp(keyName, "ALT") == 0)            return XK_Alt_L;
    if (strcmp(keyName, "HOME") == 0)           return XK_Home;
    if (strcmp(keyName, "END") == 0)            return XK_End;
    if (strcmp(keyName, "TAB") == 0)            return XK_Tab;
    if (strcmp(keyName, "RETURN") == 0)         return XK_Return;
    if (strcmp(keyName, "CAPSLOCK") == 0)       return XK_Caps_Lock;

    // 方向键
    if (strcmp(keyName, "LEFT") == 0)           return XK_Left;
    if (strcmp(keyName, "RIGHT") == 0)          return XK_Right;
    if (strcmp(keyName, "UP") == 0)             return XK_Up;
    if (strcmp(keyName, "DOWN") == 0)           return XK_Down;

    // 小键盘
    if (strcmp(keyName, "NUMPAD_Decimal") == 0)  return XK_KP_Decimal;
    if (strcmp(keyName, "NUMPAD_Multiply") == 0) return XK_multiply;
    if (strcmp(keyName, "NUMPAD_Add") == 0)      return XK_KP_Add;
    if (strcmp(keyName, "NUMPAD_Divide") == 0)   return XK_KP_Divide;
    if (strcmp(keyName, "NUMPAD_Minus") == 0)    return XK_KP_Subtract;
    if (strcmp(keyName, "NUMPAD_0") == 0)        return XK_KP_0;
    if (strcmp(keyName, "NUMPAD_1") == 0)        return XK_KP_1;
    if (strcmp(keyName, "NUMPAD_2") == 0)        return XK_KP_2;
    if (strcmp(keyName, "NUMPAD_3") == 0)        return XK_KP_3;
    if (strcmp(keyName, "NUMPAD_4") == 0)        return XK_KP_4;
    if (strcmp(keyName, "NUMPAD_5") == 0)        return XK_KP_5;
    if (strcmp(keyName, "NUMPAD_6") == 0)        return XK_KP_6;
    if (strcmp(keyName, "NUMPAD_7") == 0)        return XK_KP_7;
    if (strcmp(keyName, "NUMPAD_8") == 0)        return XK_KP_8;
    if (strcmp(keyName, "NUMPAD_9") == 0)        return XK_KP_9;

    KeySym keySym = XStringToKeysym(keyName);
    if (keySym == -1) return -1;
    return keySym;
}
#endif
